  ...
  async onload() {
    this.registerEvent(
      this.app.metadataCache.on("changed", async (current_file: TFile, data: string, cache: CachedMetadata) => {
        ...
        if (Object.keys(current_blocks).length > 0) {
          ...
          files.forEach(async (file) => {
            ...
            if (file_cache && file_cache.blocks) {
              ...
              for (const matched_block of matched_blocks) {
                ...
                if (current_block) {
                  const matched_contents = (await this.app.vault.adapter.read(file.path)).split("\n");
                  const updated_line = current_contents[current_block.position.start.line];
                  if (matched_contents[matched_block.position.start.line].contains("^" + matched_block.id)) {
                    const replace_line = matched_contents.splice(matched_block.position.start.line, 1, updated_line)[0];
                    if (replace_line != updated_line) {
                      await this.app.vault.adapter.write(file.path, matched_contents.join("\n"));
                    }
                  }
                }
              }
